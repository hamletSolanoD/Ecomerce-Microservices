...
const MyServices = () => {
  ...
  const loadStatistics = async () => {
    try {
      const stats = await cartService.getStatistics();
      setStatistics(stats);
    } catch (err) {
              ...
              <div className="flex justify-between items-start mb-4">
                <div className="flex-1">
                  <h3 className="text-lg font-semibold text-gray-900">
                    {subscription.servicio?.nombre || `Servicio ${subscription.servicioId}`}
                  </h3>
                  <p className="text-gray-600">{subscription.servicio?.descripcion}</p>
                </div>
